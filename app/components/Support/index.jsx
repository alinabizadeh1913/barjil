...
const Support = (props) => {
  ...
  const handleSubmit = (e) => {
    ...
    const svg = document.querySelector(
      "#support .form-wrapper form button svg path"
    );
    const circleLtr = document.querySelector(
      "#support .form-wrapper form .circle-wrapper-ltr"
    );
    const circleRtl = document.querySelector(
      "#support .form-wrapper form .circle-wrapper-rtl"
    );
    const form = document.querySelector("#support .form-wrapper form");

    const nameError = document.querySelector(
      "#support .form-wrapper form .input-g .error-success .name-error"
    );
    const familyError = document.querySelector(
      "#support .form-wrapper form .input-g .error-success .family-error"
    );
    const phoneError = document.querySelector(
      "#support .form-wrapper form .input-g .error-success .phone-error"
    );
    const requestError = document.querySelector(
      "#support .form-wrapper form .input-g .error-success .request-error"
    );
    const requestSuccess = document.querySelector(
      "#support .form-wrapper form .input-g .error-success .request-success"
    );

    const nameLabel = document.querySelector(
      "#support .form-wrapper form .input-g .input-label label .name"
    );
    const familyLabel = document.querySelector(
      "#support .form-wrapper form .input-g .input-label label .family"
    );
    const phoneLabel = document.querySelector(
      "#support .form-wrapper form .input-g .input-label label .phone"
    );
    const requestLabel = document.querySelector(
      "#support .form-wrapper form .input-g .input-label label .request"
    );
    ...
    if (count == 3) {
      if (input.value.length > 0) {
        ...
        axios.post(process.env.NEXT_PUBLIC_BASE_URL + "settings/contact/", {
  ...
  return (
    <section id="support" className="mt-20">
      <div
        className={`${
          props.hasContainer === "true" ? "wrapper mx-auto" : "w-full"
        } ${
          props.constant ? "support-constant" : ""
      ...
      >
        <div className={`title md:w-[79%] px-6 md:p-0`}>
          <h1
            ...
            } text-4xl md:text-5xl text-[#006400] mb-6 text-center md:text-start`}
          >
            {language == "en"
          ...
          <p className="text-[#707070] flex justify-center md:justify-start">
            {language == "en"
              ...
              : language == "ar"
              ? "أخبرنا بما تحتاجه لكي نتمكن من مساعدتك."
              : "ہمیں بتائیں کہ آپ کو کس چیز کی مدد چاہیے تاکہ ہم آپ کی مدد کر سکیں."}
            <svg
              width="66"
              height="127"
              viewBox="0 0 96 157"
        ...
        <div className="w-full flex flex-wrap justify-end items-center md:mt-[-115px]">
          <div className="w-full md:w-7/12 mb-[50px] md:mb-0">
            <div className="form-wrapper md:overflow-hidden">
              ...
              >
                <div
                  className={`circle-wrapper-ltr hidden ${
                    language == "en" ? "md:flex" : "md:hidden"
                ...
                <div
                  className={`circle-wrapper-rtl hidden ${
                    language == "en" ? "md:hidden" : "md:flex"
                  } relative justify-center items-center`}
                >
                  <div className="circle-number absolute w-[20px] h-[20px] md:w-[35px] md:h-[35px] rounded-full text-center bg-[#32CD32] flex justify-center items-center left-[-10%] md:left-[-8%]">
                    <span className="text-lg font-black text-white">1</span>
                  ...
                  <div className="flex flex-wrap items-center justify-center">
                    <div className="input-wrapper mx-2 md:mx-3 w-[65%] md:w-[75%] mb-3 relative flex justify-center">
                      <input
                        ...
                        name="name"
                        id="support-input"
                        className="text-[#757575] rounded-lg p-2 border-2 border-[#b2b2b2] bg-transparent outline-0 w-full"
                        required
                      />
                      ...
                      <div className="input-label absolute bottom-[32px] left-0 right-0">
                        <label
                          ...
                          className="text-[#757575] text-sm cursor-pointer"
                        >
                          <span className="name">
                            {language == "en"
                              ? "first name"
                      ...
                      <div className="error-success text-center absolute top-[60px] select-none">
                        ...
                        <div className="request-error text-red-500 text-sm font-bold hidden">
                          {language == "en"
                            ...
                            : language == "ar"
                            ? "من فضلك أدخل طلبك"
                            : "براہ کرم اپنی درخواست درج کریں"}
                        </div>
                        <div className="request-success text-[#32cd32] text-sm font-bold mt-2 hidden ">
                          {language == "en"
                            ? "your request has been sent. our experts will call you soon"
                            : language == "ar"
                            ? "تم إرسال طلبك. سيتصل بك خبراؤنا قريبًا."
                            : "آپ کی درخواست بھیج دی گئی ہے۔ ہمارے ماہرین جلد ہی آپ کو کال کریں گے۔"}
                        </div>
                      </div>
                    </div>
                    <button
                      ...
                      className="border-2 border-[#32CD32] rounded-lg py-[14px] px-3 flex items-center justify-center md:w-[44px] md:h-[44px] duration-300 hover:bg-[#32cd32] mb-3 bg-transparent"
                    >
          ...
          <div className="w-9/12 md:w-5/12 px-4">
            <div
              className={`image relative flex overflow-hidden ${
                language == "en" ? "justify-end" : "lang-other justify-start"
              }`}
            >
              ...
              <img
                src="/img/boy.png"
                alt=""
                className="boy-image absolute right-0 bottom-0"
              />
            </div>
          </div>
        </div>
