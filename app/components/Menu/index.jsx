    ...
    const handleClickOutside = (e) => {
      if (langWrapRef.current && !langWrapRef.current.contains(e.target)) {
        const languageList = document.querySelector(
          "#menu .change-language .language-list"
        );
    ...
    >
      <div className="container mx-auto">
        <div className="flex flex-wrap items-center justify-between md:justify-normal">
          ...
          <div className="w-8/12 md:w-8/12 hidden md:block md:order-2">
            <div className="links flex flex-wrap justify-center items-center">
              <Link
                ...
                className="mb-1 mx-[16px] text-sm"
              >
                {language == "en"
                  ? "Home"
                  : language == "ar"
          ...
          <div
            className={`w-4/12 md:w-1/12 ${
              language == "en" ? "order-1" : "order-3 justify-end"
            } md:order-3 flex md:justify-center`}
          >
            <div
              className={`change-language flex justify-center relative`}
              ref={langWrapRef}
            >
              <div
                className={`lang-wrap flex ${
                  language == "en" ? "" : "flex-row-reverse md:flex-row"
                } justify-center items-center relative`}
                onClick={showHideLanguageList}
              >
                <div className="language cursor-pointer">
                  {language == "en" ? (
                    <EnglishFlagIcon width="25" height="25" />
                  ) : language == "ur" ? (
                    <PakistanFlagIcon width="25" height="25" />
                  ) : (
                    <ArabicFlagIcon width="25" height="25" />
                  )}
                </div>
          ...
          <div
            className={`w-4/12 flex ${
              language == "en" ? "order-3 justify-end" : "order-1 justify-start"
            } md:hidden`}
          >
            <div
